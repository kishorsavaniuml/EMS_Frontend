...
function Employees() {
  ...
  const deleteClickHandler = async (userId) => {
    try {
      ...
      setEmployees((prev) => {
        return prev.filter((employee) => employee._id != userId);
      });
    } catch (err) {
      console.log(err);
              ...
              <table className="w-full">
                ...
                <tbody className="text-left">
                  {employees.map((employee) => {
                    return (
                      <tr
                        className="border-b border-gray-100 [&>td]:px-5 [&>td]:py-2"
                        key={employee._id}
                      >
                        <td>{orderNumber++}</td>
                        ...
                        <td className="capitalize">{employee.lastname}</td>
                        <td>{employee.email}</td>
                        <td className=" capitalize text-right">
                          <button

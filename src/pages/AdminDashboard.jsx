...
function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchAdminDashboardData = async () => {
      try {
        const adminDashboardData = await axios.get(
          `${SERVER_URL}/api/admin-dashboard`,
          { withCredentials: true },
        );

        setDashboardData((prev) => ({
          ...prev,
          ...adminDashboardData.data.data,
        }));
      } catch (err) {
      ...
      <AdminLayOut>
        <div className="h-full flex-1">
          ...
          <div className="main px-4 py-3 flex gap-3 flex-col bg-gray-50 mt-1 h-full">
            <h2 className="font-semibold text-gray-900 py-3">Dashboard</h2>
                  ...
                  <table>
                    <thead className="text-gray-800 text-sm">
                      <tr className="border-b border-gray-300">
                        <th className="w-1/5 py-1.5 text-start">
                          Emoloyee Name
                        </th>
